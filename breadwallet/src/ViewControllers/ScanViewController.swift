   ...
   /* func metadataOutput(_ captureOutput: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {*/
    ...
    func handleURI(_ uri: String) {
        if self.currentUri != uri {
            ...
            self.currentUri = uri
            print("QR content detected result0: \(uri)")
            let cleanAddress = uri.replacingOccurrences(of: "bastoji:", with: "")
            let result = QRCode(content: cleanAddress)
            print("QR content detected result1: \(result)")
            guard .invalid != result else {
                guide.state = .negative
                return
            }
            print("QR content detected result2: \(result)")
