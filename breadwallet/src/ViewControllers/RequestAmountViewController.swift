...
class RequestAmountViewController : UIViewController {
    ...
    private func setData() {
        ...
        //qrCode.image = UIImage.qrCode(data: "\(address.text!)".data(using: .utf8)!, color:
        let prefixAddress="bastoji:"+address.text!
        qrCode.image = UIImage.qrCode(data: "\(prefixAddress)".data(using: .utf8)!, color: CIColor(color: .black))?
            .resize(qrSize)!
